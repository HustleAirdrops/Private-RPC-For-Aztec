...
check_node_status() {
  while true; do
    clear
    echo -e "${CYAN}🔍 Ethereum Sepolia Node Sync Status Checker${NC}"

    echo -e "${YELLOW}📡 Fetching Execution (Geth) Sync Status...${NC}"
    ...
    echo -e "\n⏲️  Auto-refreshing every 10 seconds... (Press Enter to return to menu early)"
    read -t 10 -n 1 key
